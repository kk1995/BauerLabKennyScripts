...
if exist(gsLagFile)
    ...
    for trialInd = 1:trialNum
        ...
        % save lag data
        gsLagTrialFile = fullfile(saveFileLocs(trialInd),...
            strcat(saveFileDataNames(trialInd),"-gsLagHbT-",freqStr,".mat"));
        save(gsLagTrialFile,'lagTimeHbTTrial','lagAmpHbTTrial','maskTrial','tZone','corrThr','edgeLen');
        ...
        save(gsLagTrialFile,'lagTimeG6Trial','lagAmpG6Trial','maskTrial','tZone','corrThr','edgeLen');
        ...
        p(1,2,1).select(); imagesc(lagTimeG6Trial,[-tLim tLim]); axis(gca,'square');
        xlim([1 size(data,1)]); ylim([1 size(data,2)]); colormap('jet');
        set(gca,'ydir','reverse'); colorbar;
...
set(gca,'ydir','reverse'); colorbar;
set(gca,'XTick',[]); set(gca,'YTick',[]); title('HbT Lag Amp');
p(1,2,1).select(); set(gca, 'Color', [0 0 0]); imagesc(nanmean(lagTimeG6,3),'AlphaData',alphaData,[-tLim tLim]); axis(gca,'square');
xlim([1 size(data,1)]); ylim([1 size(data,2)]); colormap('jet');
set(gca,'ydir','reverse'); colorbar;
set(gca,'XTick',[]); set(gca,'YTick',[]); title('G6 Lag Time (s)');
