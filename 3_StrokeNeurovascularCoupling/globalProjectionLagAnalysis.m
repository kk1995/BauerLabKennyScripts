function globalProjectionLagAnalysis(varargin)
...
%         lagData = reshape(filtData,size(filtData,1)*size(filtData,2),size(filtData,3));
        ...
        for specInd = 1:4
            ...
            t5 = tic;
            [lagTime,lagAmp] = globalProjectionLag(squeeze(sum(lagData(:,species,:),2)),edgeLen,round(tZone*sR));
            disp(['Took ' num2str(toc(t5)) 'seconds.']);
            
            % adjust lag time to frame rate
            lagTime = lagTime./sR;
            
            % take the mean of lag
            lagTime = nanmean(lagTime,1);
            lagAmp = nanmean(lagAmp,1);
            ...
            lagMat{specInd} = cat(3,lagMat{specInd},lagMatRun);
            ampMat{specInd} = cat(3,ampMat{specInd},ampMatRun);
