dataFile2 = "L:\ProcessedData\avgBilateral_gsr.mat";
...
yvFC = reshape(regionData.yvFC,128^2,[]);
ovFC = reshape(regionData.ovFC,128^2,[]);
odFC = reshape(regionData.odFC,128^2,[]);

yvAvg = nan(numel(labels),size(yvFC,2));
ovAvg = yvAvg; odAvg = yvAvg;

for i = 1:numel(labels)
    yvAvg(i,:) = nanmean(yvFC(atlas == i,:),1);
    ovAvg(i,:) = nanmean(ovFC(atlas == i,:),1);
    odAvg(i,:) = nanmean(odFC(atlas == i,:),1);   
end
