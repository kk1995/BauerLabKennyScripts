function analyzeDotLagZachProcessed(excelFile,rows,varargin)
...
if exist(lagFile)
    ...
    for trialInd = 1:runNum
        ...
        if ~exist(fullfile(saveFileLoc,runsInfo(trialInd).recDate))
            mkdir(fullfile(saveFileLoc,runsInfo(trialInd).recDate));
        end
        
        saveFilePrefix = fullfile(saveFileLoc,runsInfo(trialInd).recDate,[runsInfo(trialInd).recDate,...
            '-',runsInfo(trialInd).mouseName,'-',runsInfo(trialInd).session,...
            num2str(runsInfo(trialInd).run)]);
        
        dotLagFile = strcat(saveFilePrefix,"-dotLagHbTG6-",postFix,".mat");
        dotLagFigFile = strcat(saveFilePrefix,"-dotLagHbTG6-",postFix,".fig");
        
        maskTrial = load(runsInfo(trialInd).saveMaskFile);
        maskTrial = maskTrial.xform_mask;
        if exist(dotLagFile)
            load(dotLagFile);
        else
            dataFile = fullfile(runsInfo(trialInd).saveFolder,...
                [runsInfo(trialInd).recDate,'-',runsInfo(trialInd).mouseName,...
                '-dataGCaMP-',runsInfo(trialInd).session,...
                num2str(runsInfo(trialInd).run),'.mat']);
            data = load(dataFile);
            xform_datahb = data.oxy + data.deoxy;
            xform_datafluorCorr = data.gcamp6corr;
            
            fs = 16.8;
            time = 1:size(xform_datafluorCorr,3); time = time./fs;
