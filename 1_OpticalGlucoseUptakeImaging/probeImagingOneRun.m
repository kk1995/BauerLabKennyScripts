...
tiffFileName = "L:\181121\181121-ProbeW7M2-Post.tif";
...
sessionInfo.lowpass = 2;

%% get gcamp and hb data

% [raw, time, xform_hb, xform_probe, xform_probeCorr, isbrain, xform_isbrain, markers] ...
%     = probe.probeImaging(tiffFileName, systemInfo, sessionInfo,'darkTime',10);

% % if brain mask and markers are available:
[raw, time, xform_hb, xform_probe, xform_probeCorr, isbrain, xform_isbrain, markers] ...
    = probe.probeImaging(tiffFileName, systemInfo, sessionInfo, isbrain, markers,'darkTime',10);
...
xform_hbAvg = mouse.preprocess.blockAvg(xform_hb,time,60,60);
xform_probeAvg = mouse.preprocess.blockAvg(xform_probe,time,60,60);
xform_probeCorrAvg = mouse.preprocess.blockAvg(xform_probeCorr,time,60,60);