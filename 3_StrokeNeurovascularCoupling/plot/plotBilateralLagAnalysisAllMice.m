...
for i = 1:numel(D)
    ...
    ampMatMean = cat(4,ampMatMean,fileData.ampMouse);
...
if useContour
    ...
    save(contourFile,'contourMask');
else
    contourMask = true(size(lagMatMean,1),size(lagMatMean,2));
    contourBounds = [1 size(lagMatMean,2); 1 size(lagMatMean,1)];
end

mouseNum = size(lagMatMean,4);
speciesNum = size(lagMatMean,3);
...
for species = 1:speciesNum
    ...
    for mouse = 1:mouseNum
        ind = mouse+(species-1)*mouseNum;
        subplot(speciesNum,mouseNum,ind);
        ...
        plotData = squeeze(lagMatMean(:,:,species,mouse));
        alphaData = maskMean(:,:,mouse);
...
for species = 1:speciesNum
    ...
    for mouse = 1:mouseNum
        ...
        plotData = squeeze(ampMatMean(:,:,species,mouse));
        alphaData = maskMean(:,:,mouse);
        alphaData(alphaData < 1) = 0; % at least 50% of mice need to have data in that pix
        alphaData(~contourMask) = 0;
...
for species = 1:speciesNum
    ...
    plotData = nanmean(lagMatMean,4);
    plotData = squeeze(plotData(:,:,species));
    alphaData = nanmean(maskMean,3);
    alphaData(alphaData < 1) = 0; % at least 50% of mice need to have data in that pix
    alphaData(~contourMask) = 0;
