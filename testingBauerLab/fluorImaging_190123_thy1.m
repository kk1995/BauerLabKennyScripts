...
for mouseInd = 1:2
    ...
    for run = 1:3
        disp(num2str(run));
        fileName = strcat("\\10.39.168.176\RawData_East3410\190123\190123-A20M",num2str(mouseInd),"-stim",num2str(run),".tif");
        saveFileName = strcat("D:\data\none\190123-A20M",num2str(mouseInd),"-stim",num2str(run),".mat");
        maskFileName = strcat("D:\data\none\190123-A20M",num2str(mouseInd),"-LandmarksandMask.mat");
        ...
        roiMap = xform_hbStim(:,:,1);
        peakCandidateCoor = mouse.plot.circleCoor(seedCoor,10);
        ...
        candidateCoor = mouse.plot.circleCoor(centerCoor,10);
...
%     imagesc(sum(plotData(:,:,speciesInd{i}),3),'AlphaData',xform_isbrain>0,cLim(i,:));
    imagesc(sum(plotData(:,:,speciesInd{i}),3),cLim(i,:));
