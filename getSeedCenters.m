...
for roiIndex = roiIndices
    ...
    roiAmpOxy = reshape(nanmean(lagAmpOxy(roiLoc,:),1),size(roiLocs));
    ...
    ax = mouse.plot.plotBrain(ax,roiAmpOxy,mask,[0 1],cMap,addCBar);
...
for roiIndex = roiIndices
    ...
    roiLoc = roiLocs == roiIndex;
    seedCenter = seedCenters(roiIndex==roiIndices,:);
    roiAmpOxy = reshape(nanmean(lagTimeOxy(:,roiLoc),2),size(roiLocs));
    ...
    ax = mouse.plot.plotBrain(ax,roiAmpOxy,mask,[-0.5 0.5],cMap,addCBar,-0.002);
    ax = mouse.plot.plotNodes(ax,seedCenter,1,[0 1],cMap2,32);
    hold on; title(num2str(roiIndex));
    set(gca,'xtick',[]); set(gca,'ytick',[]);
end