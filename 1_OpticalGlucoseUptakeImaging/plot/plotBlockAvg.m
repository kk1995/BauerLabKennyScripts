...
avgResponseFileName = 'noProbeAvgResponseW5M1.mat';
...
if exist(fullfile(saveFolder,avgResponseFileName),'file')
    ...
    for mouseInd = 1:numel(mouseList)
        ...
        xform_hbCell = mouseAnalysis.preprocess.catByTime(xform_hb,t_hb,timeBounds);
        xform_fluorCell = mouseAnalysis.preprocess.catByTime(xform_fluor,t_fluor,timeBounds);
        xform_fluorCorrCell = mouseAnalysis.preprocess.catByTime(xform_postFluorCorr,t_fluor,timeBounds);
        for i = 1:numel(xform_fluorCell)
            xform_hbCell{i} = mean(xform_hbCell{i},4);
