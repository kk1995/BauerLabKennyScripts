dataFile = "L:\ProcessedData\deborah\avgBilateral_gsr.mat";
iterNum = 2000;
...
ch = colorbar; ylabh = ylabel(ch,'z(r)');
...
histogram(yvAll,'BinWidth',0.05,'EdgeColor','auto','FaceAlpha',0.33,'Normalization','probability'); hold on;
histogram(ovAll,'BinWidth',0.05,'EdgeColor','auto','FaceAlpha',0.33,'Normalization','probability');
histogram(odAll,'BinWidth',0.05,'EdgeColor','auto','FaceAlpha',0.33,'Normalization','probability');
xlabel('z(r)');
ylabel('probability');
legend('YV','OV','OD');
...
text(1.5,sigTextY + 0.025,'Drug Effect:','FontSize',10,'Color','k','HorizontalAlignment','right');
text(1.5,sigTextY + 0.025 -0.13,'Age Effect:','FontSize',10,'Color','k','HorizontalAlignment','right');

xticks(2:numel(scatterLabels)+1);
xticklabels(scatterLabels);
xlim([0.5 numel(scatterLabels)+1.5]);
ylabel('Average correlation z(r)','FontSize',10);
...
title('Regional homotopic connectivity','FontSize',12);