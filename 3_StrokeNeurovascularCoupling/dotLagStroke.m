function dotLagStroke(varargin)
...
if nargin < 4
    species = [1 4]; % which species of data to use
else
    species = varargin{4};
end
...
for mouse = 1:fileNumel
    ...
    for run = 1:numel(runDir)
        ...
        dataRun = dataRun(:,:,species,:);
        
        % lag analysis
        [lagRun, ampRun] = dotLagFile(squeeze(dataRun(:,:,1,:)),squeeze(dataRun(:,:,2,:)),sR,fRange,tZone,false,maskRun,corrThr);
    ...
    for fInd = 1:fNumel
        ...
        metaData.lagType = 'dot';
        metaData.species = speciesName(species);
        metaData.tZone = tZone(fInd);
        ...
        for s = 1:numel(species)
            figNameExt = [figNameExt '_' speciesName{species(s)}];
        end
