...
for f = 1:numel(wavelength)
    ...
    dp(f) = (c/musp(f))*(1./(2*gamma.*sqrt(mua(f)*c))).*(1+(3/c).*mua(f).*gamma.^2);
end
...
%     musp(f) = 10;
    mua(f) = log(10)*conc*extCoeff(f,:)';
    gamma = sqrt(c) ./ sqrt(3 * (mua(f) + musp(f)));
    dp2(f) = (c/musp(f))*(1./(2*gamma.*sqrt(mua(f)*c))).*(1+(3/c).*mua(f).*gamma.^2);
end
...
legend('hillman dp','our dp','our dp with hillman musp');